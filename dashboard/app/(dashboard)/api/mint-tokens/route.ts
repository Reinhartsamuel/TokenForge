...
async function getDb() {
  ...
  try {
    ...
    const { transactions, tokens } = await import("@/db/schema");
    return { db, transactions, tokens };
  } catch {
    return null;
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Append NOOP verifier as a remaining account — SSTS verify_by_programs expects
    // verification program accounts at the end of the instruction's account list
    (mintIx.accounts as any[]).push({
      value: NOOP_VERIFICATION_PROGRAM_ID,
      isWritable: false,
    });
    ...
    if (dbClient) {
      try {
        const { db, transactions, tokens } = dbClient;
        ...
        console.error("[api/mint-tokens] DB write error (non-fatal):", dbError);
      }
    }
